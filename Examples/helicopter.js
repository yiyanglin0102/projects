...
export class Helicopter extends GrObject {
    ...
    constructor(params={}) {
        ...
        let material = new THREEmod.MeshStandardMaterial({color:"orange",transparent:true,side : THREEmod.DoubleSide});
   ...
   advance(delta,timeOfDay) {
        ...
        if (this.pads.length) {
            switch(this.state) {
                ...
                case 2:         // descend
                        this.helicopter.position.y -= deltaSlowed;
                        if(this.helicopter.position.x==0){
                        ...
                        else if (this.helicopter.position.y <= helipady) {
                            this.helicopter.position.y = helipady;
                            this.state = 3;
                            this.delay = 1+Math.random();
                        }      
                    // if(this.helicopter.position.y>=helipady){
                    //     this.helicopter.position.y-=deltaSlowed;
                    //     this.state = 3;
                    // this.delay = 1+Math.random();
                    // }
